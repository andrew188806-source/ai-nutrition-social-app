...
if (uiImports.length) fail("UI does not import Consumer Meals or SDK", "Mobile UI must not be wired to Consumer Meals in Phase 2C.", { matches: uiImports });
...
const navigationImports = walk(path.join(root, "apps", "mobile", "app"), (file) => file.endsWith(".ts") || file.endsWith(".tsx"))
  ...
  .map(relative);
if (navigationImports.length) fail("Navigation remains unchanged", "Phase 2C must not wire routes/navigation to Consumer Meals.", { matches: navigationImports });
...
async function fakeMealWriteTests() {
  ...
  if (defaults.authSource !== "mock" || defaults.mealRecordsSource !== "mock" || defaults.supabaseAuthEnabled || defaults.supabaseWritesEnabled || defaults.mealRecordWritesEnabled || defaults.issues.length) {
