...
if (uiImports.length) fail("UI does not import Consumer Meals or SDK", "Mobile UI must not be wired to Consumer Meals in Phase 2B.", { matches: uiImports });
...
const navigationImports = walk(path.join(root, "apps", "mobile", "app"), (file) => file.endsWith(".ts") || file.endsWith(".tsx"))
  .filter((file) => /consumer-meals|mealRecordsSource|MEAL_RECORDS_SOURCE/.test(fs.readFileSync(file, "utf8")))
  .map(relative);
if (navigationImports.length) fail("Navigation remains unchanged", "Phase 2B must not wire routes/navigation to Consumer Meals.", { matches: navigationImports });
