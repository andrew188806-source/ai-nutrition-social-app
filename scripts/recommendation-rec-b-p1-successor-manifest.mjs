import crypto from "node:crypto";

export const RECBP1_BASELINE = "a0015cf845381d87835b2d446065109c0ffcbf0e";
export const RECBP1_COMMIT_SUBJECT = "Establish private Taste normalization authority";
export const RECBP1_MIGRATION =
  "supabase/migrations/20260829010000_private_taste_normalization_authority.sql";

export const RECBP1_NPM_KEYS = Object.freeze([
  "test:recommendation-rec-b-p1",
  "test:recommendation-rec-b-p1-smoke",
  "test:recommendation-rec-b-p1-mutations",
  "test:recommendation-rec-b-p1-postgres"
]);

export const RECBP1_PATHS = Object.freeze([
  "docs/recommendation/rec-b-p1-user-taste-normalization-authority.md",
  "package.json",
  "packages/shared/src/domain/index.ts",
  "packages/shared/src/domain/user-taste-normalization/index.ts",
  "packages/shared/src/domain/user-taste-normalization/privateTasteNormalization.ts",
  "scripts/recommendation-rec-b-p0-guard.mjs",
  "scripts/recommendation-rec-b-p0-postgres-apply.mjs",
  "scripts/recommendation-rec-b-p1-guard.mjs",
  "scripts/recommendation-rec-b-p1-mutations.mjs",
  "scripts/recommendation-rec-b-p1-postgres-apply.mjs",
  "scripts/recommendation-rec-b-p1-smoke.mjs",
  "scripts/recommendation-rec-b-p1-successor-manifest.mjs",
  "scripts/geo-recommendation-geo-1c-guard.mjs",
  "scripts/recommendation-rec-a-guard.mjs",
  "scripts/social-candidate-sr2g-f-guard.mjs",
  "scripts/social-candidate-sr2g-g-guard.mjs",
  "scripts/social-interest-sr2c-r1-guard.mjs",
  "scripts/social-taste-sr1d-guard.mjs",
  "scripts/taste-foundation-ts2d-guard.mjs",
  RECBP1_MIGRATION
].sort());

const same = (left, right) => left.length === right.length
  && left.every((value, index) => value === right[index]);

export function classifyRecbp1Lifecycle(input) {
  const worktree = [...input.worktreePaths].sort();
  const delta = [...input.deltaPaths].sort();
  const candidate = input.head === RECBP1_BASELINE
    && input.originHead === RECBP1_BASELINE
    && input.behind === 0
    && input.ahead === 0
    && input.stagedPaths.length === 0
    && !input.deleted
    && same(worktree, RECBP1_PATHS);
  const frozenShape = input.parent === RECBP1_BASELINE
    && input.stagedPaths.length === 0
    && input.worktreePaths.length === 0
    && !input.deleted
    && same(delta, RECBP1_PATHS);
  const frozenLocal = frozenShape
    && input.originHead === RECBP1_BASELINE
    && input.behind === 0
    && input.ahead === 1;
  const frozenPushed = frozenShape
    && input.originHead === input.head
    && input.behind === 0
    && input.ahead === 0;
  const phase = candidate ? "candidate" : frozenLocal ? "frozen_local" : frozenPushed ? "frozen_pushed" : "invalid";
  return Object.freeze({ valid: phase !== "invalid", phase, manifest: candidate ? worktree : delta });
}

export function createRecbp1Manifest(readFile) {
  const entries = RECBP1_PATHS.map((file) => ({
    path: file,
    sha256: crypto.createHash("sha256").update(readFile(file)).digest("hex")
  }));
  const aggregateSha256 = crypto.createHash("sha256")
    .update(entries.map((entry) => `${entry.path}\0${entry.sha256}`).join("\n"))
    .digest("hex");
  return Object.freeze({ entries: Object.freeze(entries), aggregateSha256 });
}
