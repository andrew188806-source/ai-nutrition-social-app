...
check("E only bounded P3B-P3F writer keys may enter current application vocabulary", management.every((key) => ["admin.management.staff.account.write", "admin.management.staff.delegation.write", "admin.management.staff.console_admission.write", "admin.management.staff.permission.write"].includes(key) || (p3iPhase && ["admin.management.read", "admin.management.permissions.read", "admin.management.staff.read"].includes(key)) || !fs.readFileSync("apps/admin-web/auth/admin-current-permission-vocabulary.ts", "utf8").includes(`\"${key}\"`)));
