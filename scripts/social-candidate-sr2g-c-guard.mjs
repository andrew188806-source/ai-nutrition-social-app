...
try {
  ...
  const successorScriptKeys = ["test:social-candidate-sr2g-b-r1", "test:social-candidate-sr2g-b-r1-smoke", "test:social-candidate-sr2g-b-r1-mutations", "test:social-candidate-sr2g-b-r1-development-acceptance"];
  ...
  const sr2gcMigrationFiles = migrationFiles.filter((f) => !SR2GBR1_SUCCESSOR_PATHS.includes(`supabase/migrations/${f}`));
  ...
  check("11. no pre-existing migration changed", lines(git(["diff", "--name-only", SR2GC_BASELINE, "--", "supabase/migrations"])).filter((e) => e !== SR2GC_MIGRATION && !SR2GBR1_SUCCESSOR_PATHS.includes(e)).length === 0);
  ...
  check("64. supabase/config.toml is untouched, so no endpoint is registered", !SR2GC_SUCCESSOR_PATHS.includes("supabase/config.toml") && !SR2GBR1_SUCCESSOR_PATHS.includes("supabase/config.toml"));
