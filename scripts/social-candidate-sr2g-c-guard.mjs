...
try {
  ...
  for (const key of Object.keys(packageScripts)) delete packageWithoutSr2gc.scripts[key];
  ...
  // --- lifecycle / manifest ---------------------------------------------------------------------
  check("1. lifecycle is exactly candidate, frozen-unpushed or frozen-pushed from SR-2G-B authority", lifecycle.valid, { phase: lifecycle.phase, head: state.head, ahead: state.ahead });
  ...
  check("10. SR-2G-C adds exactly one migration and touches no other", SR2GC_SUCCESSOR_PATHS.filter((f) => f.startsWith("supabase/migrations/")).length === 1
    && exact(migrationFiles, [...baselineMigrations, path.basename(SR2GC_MIGRATION)].sort()));
  check("11. no pre-existing migration changed", lines(git(["diff", "--name-only", SR2GC_BASELINE, "--", "supabase/migrations"])).filter((e) => e !== SR2GC_MIGRATION).length === 0);
  ...
  check("64. supabase/config.toml is untouched, so no endpoint is registered", !SR2GC_SUCCESSOR_PATHS.includes("supabase/config.toml"));
