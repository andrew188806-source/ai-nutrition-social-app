...
try {
  ...
  check("3. the pinned authority is the exact frozen SR-2G-D freeze commit",
    ...
    && (state.originHead === SR2GE1_BASELINE || state.originHead === SR2GF_BASELINE || state.originHead === SR2GG_BASELINE || state.originHead === SR2HA_BASELINE || state.originHead === SR2HB_BASELINE || state.originHead === SR2IA_BASELINE));
  ...
  check("14. no migration is added or changed outside the enumerated SR-2G-F successor set",
    lines(git(["diff", "--name-only", SR2GE1_BASELINE, "--", "supabase/migrations"])).every((f) => SR2GF_SUCCESSOR_PATHS.includes(f) || SR2GG_SUCCESSOR_PATHS.includes(f) || SR2HA_SUCCESSOR_PATHS.includes(f) || SR2HB_SUCCESSOR_PATHS.includes(f) || SR2IA_SUCCESSOR_PATHS.includes(f))
