...
check("registry: exact B2/B3/C/D successor route transitions are recognized", () => {
  ...
  assert.deepEqual(changed, [...Object.keys(B2), ...Object.keys(B3), "menu-management", "menu-management-pending", "menu-management-data-quality", "nutrition-certification-pending", "restaurant-item-allergens", "restaurant-item-certification", "dashboard", "social-policies"].sort());
  ...
  assert.equal(Object.values(now).filter((v) => v === "LIVE").length, 8 + 9 + 6 + 4 + 2 + 2); // exact ADMIN-C1/C2 and ADMIN-D successors
});
