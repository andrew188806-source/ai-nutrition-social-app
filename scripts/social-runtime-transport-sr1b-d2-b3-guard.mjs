...
try {
  ...
  check("5. no migration outside exact SR-1C successor was added or changed",
    changedSince(baseline, "supabase/migrations").every((entry) => SR1C_SUCCESSOR_PATHS.includes(entry) || SR1D_SUCCESSOR_PATHS.includes(entry) || SR2B_SUCCESSOR_PATHS.includes(entry) || SR2C_SUCCESSOR_PATHS.includes(entry) || SR2D_SUCCESSOR_PATHS.includes(entry) || SR2E_SUCCESSOR_PATHS.includes(entry)),
