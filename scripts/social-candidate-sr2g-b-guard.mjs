...
try {
  ...
  const migrationDrift = lines(git(["diff", "--name-only", SR2GB_BASELINE, "--", "supabase/migrations"]))
    .filter((entry) => entry !== SR2GB_MIGRATION && !SR2GC_SUCCESSOR_PATHS.includes(entry) && !SR2GBR1_SUCCESSOR_PATHS.includes(entry) && !SR2GCR1_SUCCESSOR_PATHS.includes(entry) && !SR2CR1_SUCCESSOR_PATHS.includes(entry) && !SR2GD_SUCCESSOR_PATHS.includes(entry) && !SR2GE1_SUCCESSOR_PATHS.includes(entry) && !SR2GE2_SUCCESSOR_PATHS.includes(entry));
