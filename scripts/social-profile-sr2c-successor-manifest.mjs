// Validation-only exact SR-2C successor inventory. No prefix, glob or directory allowance.
import crypto from "node:crypto";

export const SR2C_BASELINE = "5e1f91581e8932e4b1b7426130037aac8db61922";

export const SR2C_SUCCESSOR_MIGRATION =
  "supabase/migrations/20260811040000_social_public_profile_projection.sql";

export const SR2C_SUCCESSOR_PATHS = Object.freeze([
  "package.json",
  "scripts/social-profile-sr2c-guard.mjs",
  "scripts/social-profile-sr2c-mutations.mjs",
  "scripts/social-profile-sr2c-smoke.mjs",
  "scripts/social-profile-sr2c-successor-manifest.mjs",
  ...
  "supabase/functions/_shared/social-profile/index.ts",
  "supabase/functions/_shared/social-profile/policy.ts",
...
export function createSr2cCanonicalManifest(readRawBytes) {
  if (typeof readRawBytes !== "function") throw new TypeError("readRawBytes must be a function");

  const entries = SR2C_SUCCESSOR_PATHS.map((path) => Object.freeze({
    path,
...
export function classifySr2cLifecycle(state) {
  ...
  const candidate =
    state.head === SR2C_BASELINE &&
    state.originHead === SR2C_BASELINE &&
    state.ahead === 0 &&
    state.behind === 0 &&
    exactPathSet(worktreePaths, SR2C_SUCCESSOR_PATHS) &&
    stagedPaths.length === 0;

  const frozenShape =
    state.head !== SR2C_BASELINE &&
    state.headParent === SR2C_BASELINE &&
    worktreePaths.length === 0 &&
    stagedPaths.length === 0 &&
    exactPathSet(headDeltaPaths, SR2C_SUCCESSOR_PATHS) &&
