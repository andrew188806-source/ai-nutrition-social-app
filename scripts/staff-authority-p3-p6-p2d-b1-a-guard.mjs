...
check("exact predecessor and local lifecycle", p3bSuccessor || candidate || frozen || b1bPhase, { head, origin, ahead, behind, status });
...
check("B1-A changes no migration", p3bSuccessor || lines(git("diff", "--name-only", PREDECESSOR, "--", "supabase/migrations")).every((file) => p3aPhase && file === P3A_MIGRATION));
...
check("hybrid composer emits legacy admission", p3bSuccessor || b1bPhase ? /resolveStaffAdminPermissionContext\(client, authority\.subject, "legacy"\)/.test(context) : /admissionAuthority: "legacy" as const/.test(hybrid) && !/admissionAuthority: "staff"/.test(hybrid));
...
check("authority selector is exact for phase", p3bSuccessor || b1bPhase ? /\| "staff";/.test(selector) && /value === "staff"/.test(selector) : unchanged("apps/admin-web/auth/admin-authority-selector.ts"));
check("selector values remain phase-exact", p3bSuccessor || b1bPhase ? /\| "legacy"[\s\S]*\| "staff_permissions_legacy_admission"[\s\S]*\| "staff";/.test(selector) : /\| "legacy"\s*\n\s*\| "staff_permissions_legacy_admission";/.test(selector) && !/\| "staff";|value === "staff"/.test(selector));
...
check("legacy admission precedes hybrid staff permissions", p3bSuccessor || b1bPhase ? context.indexOf("const authority = await resolveLegacyAdminAuthority") < context.indexOf("return resolvePermissionsForAuthority(client, authority, mode.mode)") : context.indexOf('authority.context.state !== "admin"') < context.indexOf('mode.mode === "staff_permissions_legacy_admission"'));
check("legacy base permission remains mandatory", /!authority\.context\.permissions\.includes\("admin_context\.read"\)/.test(context));
check("staff-only admission is phase-correct", p3bSuccessor || b1bPhase ? /mode\.mode === "staff"[\s\S]*resolveStaffAdminPermissionContext\(client, identity\.subject, "staff"\)/.test(context) : !/mode\.mode === "staff"|admissionAuthority: "staff" as const/.test(context));
check("hybrid still uses staff permissions only", p3bSuccessor || /permissions: staffPermissions\.permissions/.test(b1bPhase ? context : hybrid) && !/concat|union|authority\.context\.permissions[^\n]*staffPermissions/.test(b1bPhase ? context : hybrid));
...
check("B0 read selector is phase-exact", p3bSuccessor || b1bPhase ? /mode\.mode === "legacy" \? "legacy" as const : "staff"/.test(read("apps/admin-web/auth/admin-protected-read-authority.ts")) : unchanged("apps/admin-web/auth/admin-protected-read-authority.ts"));
check("B0 mutation selector is phase-exact", p3bSuccessor || b1bPhase ? /mode\.mode === "legacy" \? "legacy" as const : "staff"/.test(read("apps/admin-web/auth/admin-protected-mutation-authority.ts")) : unchanged("apps/admin-web/auth/admin-protected-mutation-authority.ts"));
...
check("permission vocabulary is byte-identical", p3bSuccessor || unchanged("apps/admin-web/auth/admin-current-permission-vocabulary.ts"));
