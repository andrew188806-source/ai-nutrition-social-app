...
const ALLOWED = new Set([...CORE, ...FIXTURES, ...SUCCESSOR_GUARDS]);
...
check("hybrid composer emits legacy admission", /admissionAuthority: "legacy" as const/.test(hybrid) && !/admissionAuthority: "staff"/.test(hybrid));
...
check("authority selector is byte-identical", unchanged("apps/admin-web/auth/admin-authority-selector.ts"));
check("selector values remain exact two", /\| "legacy"\s*\n\s*\| "staff_permissions_legacy_admission";/.test(selector) && !/\| "staff";|value === "staff"/.test(selector));
...
check("legacy admission precedes hybrid staff permissions", context.indexOf('authority.context.state !== "admin"') < context.indexOf('mode.mode === "staff_permissions_legacy_admission"'));
check("legacy base permission remains mandatory", /!authority\.context\.permissions\.includes\("admin_context\.read"\)/.test(context));
check("staff-only admission branch is absent", !/mode\.mode === "staff"|admissionAuthority: "staff" as const/.test(context));
check("hybrid still uses staff permissions only", /permissions: staffPermissions\.permissions/.test(hybrid) && !/concat|union|authority\.context\.permissions[^\n]*staffPermissions/.test(hybrid));
check("hybrid denial has no legacy fallback", /if \(staffPermissions\.state !== "ready"\) return staffPermissions;/.test(hybrid));
...
check("B0 read selector is byte-identical", unchanged("apps/admin-web/auth/admin-protected-read-authority.ts"));
check("B0 mutation selector is byte-identical", unchanged("apps/admin-web/auth/admin-protected-mutation-authority.ts"));
