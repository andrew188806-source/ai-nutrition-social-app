...
try {
  ...
  const sr2cNonSuccessorMigrations = repositoryMigrations.filter((f) => !SR2GA_SUCCESSOR_PATHS.includes(`supabase/migrations/${f}`));
  ...
  check("63. no frozen runtime or migration path has a worktree delta", [...frozenFiles.keys()].filter((file) => !SR2D_SUCCESSOR_PATHS.includes(file) && !SR2E_SUCCESSOR_PATHS.includes(file) && !SR2F_SUCCESSOR_PATHS.includes(file) && !SR2GA_SUCCESSOR_PATHS.includes(file)).every((file) => git(["diff", "--name-only", SR2C_BASELINE, "--", file]).trim() === ""));
