...
check("exact P1A predecessor through bounded P2A lifecycle", candidate || frozen || pushed || p1cPhase, { head, origin, ahead, behind, status });
...
check("migration inventory is exact for recognized phase", migrations.length === (p2aPhase ? 113 : p1cPhase ? 112 : 111), migrations.length);
check("latest migration is exact for recognized phase", migrations.at(-1) === path.basename(p2aPhase ? P2A_MIGRATION : p1cPhase ? P1C_MIGRATION : MIGRATION), migrations.at(-1));
const changedMigrations = changed.filter((file) => file.startsWith("supabase/migrations/"));
check("only exact additive migrations are introduced", JSON.stringify(changedMigrations) === JSON.stringify(p2aPhase ? [MIGRATION, P1C_MIGRATION, P2A_MIGRATION] : p1cPhase ? [MIGRATION, P1C_MIGRATION] : [MIGRATION]), changedMigrations);
check("all frozen predecessor migrations remain byte-identical", lines(git("diff", "--name-only", PREDECESSOR, "--", "supabase/migrations")).every((file) => file === MIGRATION || (p1cPhase && file === P1C_MIGRATION) || (p2aPhase && file === P2A_MIGRATION)));
...
console.log("\n" + JSON.stringify({
  suite: "staff-authority-p3-p6-p1b-guard", phase: candidate ? "candidate" : frozen ? "frozen_local" : p1cCandidate ? "p1c_candidate" : p1cFrozen ? "p1c_frozen_local" : p2aCandidate ? "p2a_candidate" : p2aFrozen ? "p2a_frozen_local" : p1cPushed ? "p1c_pushed" : pushed ? "pushed" : "invalid",
