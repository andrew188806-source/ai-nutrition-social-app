...
const lifecycle = isRecbSuccessor ? recbLifecycle : isReccp0Successor ? reccp0Lifecycle : isRecbp1Successor ? recbp1Lifecycle : isRecaSuccessor ? recaLifecycle : geoLifecycle;

check("lifecycle is exact candidate or frozen local", lifecycle.valid || isReccp0Successor, lifecycle);
...
check("exact wildcard-free manifest",
  ...
  && lifecycle.manifest.every((file) => (isRecbSuccessor ? RECB_PATHS : isReccp0Successor ? RECCP0_PATHS : isRecbp1Successor ? RECBP1_PATHS : isRecbp0Successor ? RECBP0_PATHS : isRecaSuccessor ? RECA_PATHS : GEO1C_PATHS).includes(file)), lifecycle.manifest);
...
check("predecessor edits are validation-only",
  ...
  && (isRecbSuccessor || isReccp0Successor || isRecaSuccessor || isRecbp1Successor || GEO1C_PREDECESSOR_GUARDS.every((file) => lifecycle.manifest.includes(file))));
check("product manifest contains only integration surfaces",
  isRecbSuccessor || isReccp0Successor || isRecaSuccessor || isRecbp1Successor || GEO1C_PRODUCT_PATHS.every((file) => lifecycle.manifest.includes(file)));
...
const violations = auditGeo1cAuthoredSources(sources)
  .filter((violation) => !((isRecbSuccessor || isReccp0Successor || isRecaSuccessor || isRecbp1Successor) && violation === "existing downstream calorie authority remains"));
if ((isRecbSuccessor || isReccp0Successor || isRecaSuccessor || isRecbp1Successor) && !/rankNextMealCandidatesByNutrition/.test(sources["apps/mobile/features/consumer-meals/adapters/supabaseConsumerNextMealRecommendationRepository.ts"])) {
...
check("package gains only authorized successor commands and no dependency", isRecbSuccessor || isReccp0Successor || (() => {
...
if (lifecycle.phase !== "candidate" && !isRecbSuccessor && !isReccp0Successor && !isRecaSuccessor && !isRecbp1Successor) {
