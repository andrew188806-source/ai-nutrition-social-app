...
const lifecycle = isRecbSuccessor ? recbLifecycle : isRecbp1Successor ? recbp1Lifecycle : isRecaSuccessor ? recaLifecycle : geoLifecycle;
...
check("exact wildcard-free manifest",
  ...
  && lifecycle.manifest.every((file) => (isRecbSuccessor ? RECB_PATHS : isRecbp1Successor ? RECBP1_PATHS : isRecbp0Successor ? RECBP0_PATHS : isRecaSuccessor ? RECA_PATHS : GEO1C_PATHS).includes(file)), lifecycle.manifest);
...
check("no frozen migration is modified",
  isRecbSuccessor ? migrationDelta.every((file) => file === RECBP0_MIGRATION || file === RECBP1_MIGRATION)
...
check("predecessor edits are validation-only",
  ...
  && (isRecbSuccessor || isRecaSuccessor || isRecbp1Successor || GEO1C_PREDECESSOR_GUARDS.every((file) => lifecycle.manifest.includes(file))));
check("product manifest contains only integration surfaces",
  isRecbSuccessor || isRecaSuccessor || isRecbp1Successor || GEO1C_PRODUCT_PATHS.every((file) => lifecycle.manifest.includes(file)));
...
const violations = auditGeo1cAuthoredSources(sources)
  .filter((violation) => !((isRecbSuccessor || isRecaSuccessor || isRecbp1Successor) && violation === "existing downstream calorie authority remains"));
if ((isRecbSuccessor || isRecaSuccessor || isRecbp1Successor) && !/rankNextMealCandidatesByNutrition/.test(sources["apps/mobile/features/consumer-meals/adapters/supabaseConsumerNextMealRecommendationRepository.ts"])) {
...
check("package gains only authorized successor commands and no dependency", isRecbSuccessor || (() => {
...
if (lifecycle.phase !== "candidate" && !isRecbSuccessor && !isRecaSuccessor && !isRecbp1Successor) {
