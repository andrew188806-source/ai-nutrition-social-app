...
const lifecycle = isReccSuccessor ? reccLifecycle : isRecbSuccessor ? recbLifecycle : isReccp0Successor ? reccp0Lifecycle : isRecbp1Successor ? recbp1Lifecycle : isRecaSuccessor ? recaLifecycle : geoLifecycle;

check("lifecycle is exact candidate, frozen local, or exact REC-C successor", lifecycle.valid || isReccp0Successor, lifecycle);
...
check("exact wildcard-free manifest",
  ...
  && lifecycle.manifest.every((file) => (isReccSuccessor ? RECC_PATHS : isRecbSuccessor ? RECB_PATHS : isReccp0Successor ? RECCP0_PATHS : isRecbp1Successor ? RECBP1_PATHS : isRecbp0Successor ? RECBP0_PATHS : isRecaSuccessor ? RECA_PATHS : GEO1C_PATHS).includes(file)), lifecycle.manifest);
...
check("predecessor edits are validation-only",
  ...
  && (isReccSuccessor || isRecbSuccessor || isReccp0Successor || isRecaSuccessor || isRecbp1Successor || GEO1C_PREDECESSOR_GUARDS.every((file) => lifecycle.manifest.includes(file))));
check("product manifest contains only integration surfaces",
  isReccSuccessor || isRecbSuccessor || isReccp0Successor || isRecaSuccessor || isRecbp1Successor || GEO1C_PRODUCT_PATHS.every((file) => lifecycle.manifest.includes(file)));
...
const violations = auditGeo1cAuthoredSources(sources)
  .filter((violation) => !((isReccSuccessor || isRecbSuccessor || isReccp0Successor || isRecaSuccessor || isRecbp1Successor)
...
if ((isReccSuccessor || isRecbSuccessor || isReccp0Successor || isRecaSuccessor || isRecbp1Successor) && !/rankNextMealCandidatesByNutrition/.test(recommendationRepository)) {
...
check("package gains only authorized successor commands and no dependency", isReccSuccessor || isRecbSuccessor || isReccp0Successor || (() => {
...
if (lifecycle.phase !== "candidate" && !isReccSuccessor && !isRecbSuccessor && !isReccp0Successor && !isRecaSuccessor && !isRecbp1Successor) {
