...
check(changed.every((line) => /^A\t/.test(line)) && changed.every((line) => [path.basename(H3), path.basename(H4), "20260920010000_admin_operational_read_permissions_ae1.sql", "20260920020000_admin_restaurant_operational_read_foundation_b1.sql", "20260920030000_admin_operational_review_queues_c.sql", "20260921010000_admin_dashboard_social_policy_reads_d.sql"].some((n) => line.endsWith(n)) || !line.includes("supabase/migrations")), "no historical migration was modified or removed since the baseline (only H3/H4 and the exact ADMIN-AE1, ADMIN-B1, ADMIN-C and ADMIN-D successors added)", changed);
...
const KEY_LINE = /^[+-]\s*(\{ key: "admin[._a-z]*", status: "(CURRENT|PLANNED)", description: |defineRoute\(\{ id: "(dashboard|restaurants|restaurant-detail|restaurant-about|restaurant-contact|restaurant-branches|restaurant-branch-detail|restaurant-branch-contact|restaurant-branch-hours|restaurant-branch-geo|restaurant-menus|restaurant-menu-detail|restaurant-menu-items|restaurant-branch-menu-items|restaurant-menu-item-detail|restaurant-item-nutrition|menu-management|menu-management-pending|menu-management-data-quality|nutrition-certification-pending|restaurant-item-allergens|restaurant-item-certification|social-policies)", )/;
const registryDiff = git(["diff", "-U0", BASELINE, "--", "apps/admin-web/auth/admin-route-registry.ts"]).split("\n").filter((l) => /^[+-]/.test(l) && !/^(\+\+\+|---)/.test(l));
check(registryDiff.every((l) => KEY_LINE.test(l)), "the Admin route registry is unchanged since the baseline except exact ADMIN-AE1 keys and the ADMIN-B/C/D successor route-availability lines", registryDiff.filter((l) => !KEY_LINE.test(l)));
