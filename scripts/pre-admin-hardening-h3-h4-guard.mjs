...
const KEY_LINE = /^[+-]\s*(\{ key: "admin[._a-z]*", status: "(CURRENT|PLANNED)", description: |defineRoute\(\{ id: "(restaurants|restaurant-detail|restaurant-about|restaurant-contact|restaurant-branches|restaurant-branch-detail|restaurant-branch-contact|restaurant-branch-hours|restaurant-branch-geo|restaurant-menus|restaurant-menu-detail|restaurant-menu-items|restaurant-branch-menu-items|restaurant-menu-item-detail|restaurant-item-nutrition|menu-management|menu-management-pending|menu-management-data-quality|nutrition-certification-pending)", )/;
const registryDiff = git(["diff", "-U0", BASELINE, "--", "apps/admin-web/auth/admin-route-registry.ts"]).split("\n").filter((l) => /^[+-]/.test(l) && !/^(\+\+\+|---)/.test(l));
check(registryDiff.every((l) => KEY_LINE.test(l)), "the Admin route registry is unchanged since the baseline except exact permission-key status lines (ADMIN-AE1) and the nine ADMIN-B2 plus six ADMIN-B3 plus four ADMIN-C route-availability lines", registryDiff.filter((l) => !KEY_LINE.test(l)));
