...
check("exact P3D predecessor is current or bounded successor", frozen || p3gPhase || r1Phase);
check("migration inventory is exact through P3G-R1", migrationCount === (r1Phase ? 124 : p3gPhase ? 123 : p3fPhase ? 122 : 121));
...
const expectedKeys = ["admin_audit.read", "admin_context.read", "admin.management.staff.account.write", "admin.management.staff.delegation.write", "admin.management.staff.console_admission.write", ...((p3fPhase || p3gPhase || r1Phase) ? ["admin.management.staff.permission.write"] : []), "admin_restaurant_branch.status.write"];
...
check("application vocabulary has no unexpected key", currentKeys.length === ((p3fPhase || p3gPhase || r1Phase) ? 7 : 6));
check("permission.write follows bounded P3F/P3G/P3G-R1 state", currentKeys.includes("admin.management.staff.permission.write") === (p3fPhase || p3gPhase || r1Phase));
