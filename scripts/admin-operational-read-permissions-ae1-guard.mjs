...
check("dashboard hub stays a base-Admin landing with no data; its counts key exists CURRENT and is not any route's requirement yet", () => {
  assert.deepEqual([...route("dashboard").requiredPermissions], [BASE]);
  assert.equal(route("dashboard").availability, "DEMO");
...
check("migration is the exact additive successor (name, single file, predecessor untouched)", () => {
  ...
  assert.equal(all.at(-3), path.basename(MIGRATION)); // ADMIN-B1 and ADMIN-C are the authorized later successors
  assert.equal(all.at(-2), "20260920020000_admin_restaurant_operational_read_foundation_b1.sql");
  assert.equal(all.at(-1), "20260920030000_admin_operational_review_queues_c.sql");
