// Validation-only exact SR-2B successor inventory. No prefix, glob or directory allowance.
import crypto from "node:crypto";

export const SR2B_BASELINE = "b0673368b220c18c7c83fbf2e9bed350c8423d2e";

export const SR2B_SUCCESSOR_MIGRATION =
  "supabase/migrations/20260811030000_social_exposure_entitlement_authenticated_read.sql";

export const SR2B_SUCCESSOR_PATHS = Object.freeze([
  "package.json",
  "scripts/social-exposure-sr2b-guard.mjs",
  "scripts/social-exposure-sr2b-mutations.mjs",
  "scripts/social-exposure-sr2b-smoke.mjs",
  "scripts/social-exposure-sr2b-successor-manifest.mjs",
...
export function classifySr2bLifecycle(state) {
  ...
  const candidate =
    state.head === SR2B_BASELINE &&
    state.originHead === SR2B_BASELINE &&
    state.ahead === 0 &&
    state.behind === 0 &&
    exactPathSet(worktreePaths, SR2B_SUCCESSOR_PATHS) &&
    stagedPaths.length === 0;

  const frozenShape =
    state.head !== SR2B_BASELINE &&
    state.headParent === SR2B_BASELINE &&
    worktreePaths.length === 0 &&
    stagedPaths.length === 0 &&
    exactPathSet(headDeltaPaths, SR2B_SUCCESSOR_PATHS) &&
