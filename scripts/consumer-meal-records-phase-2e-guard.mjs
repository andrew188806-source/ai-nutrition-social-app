...
else fail("migration inventory unchanged", "Phase 2E must not add or modify active migrations.", { migrationFiles });
...
async function fakeSummaryTests() {
  ...
  if (defaults.authSource !== "mock" || defaults.mealRecordsSource !== "mock" || defaults.dailyNutritionSource !== "mock" || defaults.supabaseAuthEnabled || defaults.supabaseWritesEnabled || defaults.issues.length) {
  ...
  if (!live.issues.some((issue) => issue.includes("not enabled in Consumer Runtime Phase 2E"))) throw new Error("live summary reads must remain blocked in Phase 2E");
  ...
  if (mapped.summaryDate !== "2026-07-13" || mapped.provenance !== "stored" || mapped.itemCount !== 0) throw new Error("summary mapper did not produce canonical stored summary");
