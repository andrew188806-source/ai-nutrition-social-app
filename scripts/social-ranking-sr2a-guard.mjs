...
try {
  ...
  const sr2aOwnedPaths = SR2A_SUCCESSOR_PATHS.filter((file) => !SR2B_SUCCESSOR_PATHS.includes(file) && !SR2C_SUCCESSOR_PATHS.includes(file) && !SR2D_SUCCESSOR_PATHS.includes(file) && !SR2E_SUCCESSOR_PATHS.includes(file) && !SR2F_SUCCESSOR_PATHS.includes(file) && !SR2GA_SUCCESSOR_PATHS.includes(file));
  ...
  check("2. lifecycle manifest is the exact SR-2G-A successor path set", exact(lifecycle.lifecycleManifest, SR2GA_SUCCESSOR_PATHS), { expected: SR2GA_SUCCESSOR_PATHS, actual: lifecycle.lifecycleManifest });
  ...
  check("45. all frozen SR-1D live-relevant and predecessor migration bytes retain exact SHA-256", [...frozenFiles].filter(([file]) => !SR2D_SUCCESSOR_PATHS.includes(file) && !SR2E_SUCCESSOR_PATHS.includes(file) && !SR2F_SUCCESSOR_PATHS.includes(file) && !SR2GA_SUCCESSOR_PATHS.includes(file)).every(([file, hash]) => fs.existsSync(path.join(root, file)) && sha256(file) === hash));
  ...
  check("47. no frozen migration, config or live SR-1D path has a worktree delta", [...frozenFiles.keys()].filter((file) => !SR2D_SUCCESSOR_PATHS.includes(file) && !SR2E_SUCCESSOR_PATHS.includes(file) && !SR2F_SUCCESSOR_PATHS.includes(file) && !SR2GA_SUCCESSOR_PATHS.includes(file)).every((file) => git(["diff", "--name-only", SR2A_BASELINE, "--", file]).trim() === ""));
