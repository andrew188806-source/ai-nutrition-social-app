...
try {
  ...
  const successorScriptKeys = ["test:social-exposure-sr2b", "test:social-exposure-sr2b-smoke", "test:social-exposure-sr2b-mutations", "test:social-profile-sr2c", "test:social-profile-sr2c-smoke", "test:social-profile-sr2c-mutations", "test:social-candidate-sr2d", "test:social-candidate-sr2d-smoke", "test:social-candidate-sr2d-mutations", "test:social-candidate-sr2e", "test:social-candidate-sr2e-smoke", "test:social-candidate-sr2e-mutations", "test:social-candidate-sr2e-development-mobile-smoke"];
  ...
  const sr2aOwnedPaths = SR2A_SUCCESSOR_PATHS.filter((file) => !SR2B_SUCCESSOR_PATHS.includes(file) && !SR2C_SUCCESSOR_PATHS.includes(file) && !SR2D_SUCCESSOR_PATHS.includes(file) && !SR2E_SUCCESSOR_PATHS.includes(file));
  ...
  check("45. all frozen SR-1D live-relevant and predecessor migration bytes retain exact SHA-256", [...frozenFiles].filter(([file]) => !SR2D_SUCCESSOR_PATHS.includes(file) && !SR2E_SUCCESSOR_PATHS.includes(file)).every(([file, hash]) => fs.existsSync(path.join(root, file)) && sha256(file) === hash));
  ...
  check("47. no frozen migration, config or live SR-1D path has a worktree delta", [...frozenFiles.keys()].filter((file) => !SR2D_SUCCESSOR_PATHS.includes(file) && !SR2E_SUCCESSOR_PATHS.includes(file)).every((file) => git(["diff", "--name-only", SR2A_BASELINE, "--", file]).trim() === ""));
