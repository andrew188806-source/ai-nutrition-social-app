...
check("exact P1B predecessor through bounded P2B lifecycle", p3bSuccessor || candidate || frozen || pushed || p2aPhase, { head, origin, ahead, behind, status });
check("P1C diff contains only the bounded manifest", p3bSuccessor || changed.every((file) => allowed.has(file)), changed.filter((file) => !allowed.has(file)));
...
check("migration inventory is exact for recognized phase", p3bSuccessor || migrations.length === (p3aPhase ? 117 : b0bPhase ? 116 : b0aPhase ? 115 : p2bPhase ? 114 : p2aPhase ? 113 : 112), migrations.length);
check("latest migration is exact for recognized phase", p3bSuccessor || migrations.at(-1) === path.basename(p3aPhase ? P3A_MIGRATION : b0bPhase ? B0B_MIGRATION : b0aPhase ? B0A_MIGRATION : p2bPhase ? P2B_MIGRATION : p2aPhase ? P2A_MIGRATION : MIGRATION), migrations.at(-1));
const changedMigrations = changed.filter((file) => file.startsWith("supabase/migrations/")).filter((file) => !(p3aPhase && file === P3A_MIGRATION));
check("only exact additive migrations are introduced", p3bSuccessor || JSON.stringify(changedMigrations) === JSON.stringify(b0bPhase ? [MIGRATION, P2A_MIGRATION, P2B_MIGRATION, B0A_MIGRATION, B0B_MIGRATION] : b0aPhase ? [MIGRATION, P2A_MIGRATION, P2B_MIGRATION, B0A_MIGRATION] : p2bPhase ? [MIGRATION, P2A_MIGRATION, P2B_MIGRATION] : p2aPhase ? [MIGRATION, P2A_MIGRATION] : [MIGRATION]), changedMigrations);
check("all frozen predecessor migrations remain byte-identical", p3bSuccessor || lines(git("diff", "--name-only", PREDECESSOR, "--", "supabase/migrations")).every((file) => file === MIGRATION || (p2aPhase && file === P2A_MIGRATION) || (p2bPhase && file === P2B_MIGRATION) || (b0aPhase && file === B0A_MIGRATION) || (b0bPhase && file === B0B_MIGRATION) || (p3aPhase && file === P3A_MIGRATION)));
