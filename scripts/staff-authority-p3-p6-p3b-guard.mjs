...
check("migration count is exact through bounded P3D", migrations.length === (p3dPhase ? 120 : p3cPhase ? 119 : 118), migrations.length);
check("P3B migration is followed only by exact P3C/P3D", migrations.at(-1) === path.basename(p3dPhase ? P3D : p3cPhase ? P3C : MIGRATION), migrations.at(-1));
...
check("application current vocabulary is exact through bounded P3D", JSON.stringify(vocabulary.CURRENT_ADMIN_PERMISSION_KEYS) === JSON.stringify((p3cPhase || p3dPhase) ? [...exactCurrent.slice(0, 3), "admin.management.staff.delegation.write", exactCurrent[3]] : exactCurrent), vocabulary.CURRENT_ADMIN_PERMISSION_KEYS);
check("only bounded operational management keys are current", otherManagement.every((key) => (p3cPhase || p3dPhase) && key === "admin.management.staff.delegation.write" || !vocabulary.CURRENT_ADMIN_PERMISSION_KEYS.includes(key)));
