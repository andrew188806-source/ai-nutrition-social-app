...
try {
  ...
  check("62. the repository migration set is unchanged from the baseline", exact(fs.readdirSync(path.join(root, "supabase/migrations")).filter((f) => f.endsWith(".sql")).sort(), lines(git(["ls-tree", "-r", "--name-only", SR2E_BASELINE, "--", "supabase/migrations"])).map((f) => path.basename(f))));
