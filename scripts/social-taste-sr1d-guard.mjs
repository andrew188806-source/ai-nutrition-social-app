...
function collectLifecycleState() {
  ...
  return Object.freeze({
    ...
    headDeltaEntries: head === SR2E_BASELINE ? [] : commitDeltaEntries()
  });
}
...
try {
  ...
  const lifecycle = classifySr2eLifecycle(lifecycleState);
  ...
  check("1a. candidate or frozen SR-2E successor manifest is exact and contains no unrelated path", same(lifecycle.lifecycleManifest, SR2E_SUCCESSOR_PATHS), { expected: SR2E_SUCCESSOR_PATHS, actual: lifecycle.lifecycleManifest });
