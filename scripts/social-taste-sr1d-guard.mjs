...
function collectLifecycleState() {
  ...
  return Object.freeze({
    ...
    headDeltaEntries: head === SR2CR1_BASELINE ? [] : commitDeltaEntries()
  });
}
...
try {
  ...
  const lifecycle = classifySr2cr1Lifecycle(lifecycleState);
  ...
  check("1a. candidate or frozen SR-2C-R1 successor manifest is exact and contains no unrelated path", same(lifecycle.lifecycleManifest, SR2CR1_SUCCESSOR_PATHS), { expected: SR2CR1_SUCCESSOR_PATHS, actual: lifecycle.lifecycleManifest });
