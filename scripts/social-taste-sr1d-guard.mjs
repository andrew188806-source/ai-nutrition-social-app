...
try {
  ...
  const effectiveLifecycle = recbLifecycle.valid ? recbLifecycle : recbp1Lifecycle.valid ? recbp1Lifecycle
  ...
  check("1a. candidate or frozen successor manifest is exact and contains no unrelated path",
    recbLifecycle.valid ? same(recbLifecycle.manifest, RECB_PATHS)
    ...
    { expected: recbLifecycle.valid ? RECB_PATHS : recbp1Lifecycle.valid ? RECBP1_PATHS : recbp0Lifecycle.valid ? RECBP0_PATHS : SR2GF_SUCCESSOR_PATHS,
      actual: recbLifecycle.valid ? recbLifecycle.manifest : recbp1Lifecycle.valid ? recbp1Lifecycle.manifest : recbp0Lifecycle.valid ? recbp0Lifecycle.manifest : lifecycle.lifecycleManifest });
