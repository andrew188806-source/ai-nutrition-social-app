...
function collectLifecycleState() {
  ...
  return Object.freeze({
    ...
    headDeltaEntries: head === SR2GB_BASELINE ? [] : commitDeltaEntries()
  });
}
...
try {
  ...
  const lifecycle = classifySr2gbLifecycle(lifecycleState);
  ...
  check("1a. candidate or frozen SR-2G-B successor manifest is exact and contains no unrelated path", same(lifecycle.lifecycleManifest, SR2GB_SUCCESSOR_PATHS), { expected: SR2GB_SUCCESSOR_PATHS, actual: lifecycle.lifecycleManifest });
