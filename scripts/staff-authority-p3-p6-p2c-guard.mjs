...
check("migration count is exact through bounded B0-A", migrations.length === (b0bPhase ? 116 : b0aPhase ? 115 : 114), migrations.length);
check("latest migration is exact through bounded B0-B", migrations.at(-1) === (b0bPhase ? path.basename(B0B_MIGRATION) : b0aPhase ? path.basename(B0A_MIGRATION) : "20260912050000_staff_authority_p3_p6_p2b_platform_admin_compatibility.sql"), migrations.at(-1));
check("P2C migration boundary remains frozen", lines(git("diff", "--name-only", B0A_PREDECESSOR, "--", "supabase/migrations")).every((file) => file === B0A_MIGRATION || (b0bPhase && file === B0B_MIGRATION)));
