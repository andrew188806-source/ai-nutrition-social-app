...
try {
  ...
  const successorScriptKeys = ["test:social-interest-sr2c-r1", "test:social-interest-sr2c-r1-smoke", "test:social-interest-sr2c-r1-mutations", "test:social-interest-sr2c-r1-development-acceptance", "test:social-candidate-sr2g-d", "test:social-candidate-sr2g-d-smoke", "test:social-candidate-sr2g-d-mutations", "test:social-candidate-sr2g-d-development-acceptance"];
  ...
  const sr2gcr1MigrationFiles = migrationFiles.filter((f) => !SR2CR1_SUCCESSOR_PATHS.includes(`supabase/migrations/${f}`) && !SR2GD_SUCCESSOR_PATHS.includes(`supabase/migrations/${f}`));
  ...
  check("11. no prior migration byte is modified", lines(git(["diff", "--name-only", SR2GCR1_BASELINE, "--", "supabase/migrations"])).filter((e) => e !== SR2GCR1_MIGRATION && !SR2CR1_SUCCESSOR_PATHS.includes(e) && !SR2GD_SUCCESSOR_PATHS.includes(e)).length === 0);
