...
Prevent known Haocu bugs from returning.

## Why This Matters for Haocu

- Previous issues included wrong chat back route, chat sorting, meal record sync, avatar inconsistency, restaurant card date placement.
- Regression checklist protects long-running repository work.

## MVP Requirements

- Check navigation, meal records, Meal Buddy, chat, group table, restaurant, UI, premium rules, i18n.
...
- [ ] Known regressions listed.
- [ ] Checklist run after refactors and before demos.

## Implementation Backlog

| # | Task | Priority | Acceptance Criteria |
|---:|---|---|---|
| 1 | Add navigation tests | P0 | Owner assigned, acceptance criteria written, and linked to relevant PRD/Data/API/UI docs. |
| 2 | Add meal sync tests | P0 | Owner assigned, acceptance criteria written, and linked to relevant PRD/Data/API/UI docs. |
| 3 | Add chat sorting tests | P0 | Owner assigned, acceptance criteria written, and linked to relevant PRD/Data/API/UI docs. |
| 4 | Add restaurant card tests | P1 | Owner assigned, acceptance criteria written, and linked to relevant PRD/Data/API/UI docs. |

## Known Regression Areas

- Home and full nutrition report must use the same meal record collection.
- `latestCorrectedMealRecord` must not replace the real meal record collection.
- Meal Buddy and Group Table must reference the same identity/mock user source.
- Restaurant card creation must navigate to the created card location.
- Chat list must reorder after a new message.
- Chat back button must return to the chat list, not the wrong tab.
- Restaurant card date selector belongs under the restaurant card, not at the page bottom.
- Traditional Chinese text should not be mixed with accidental hard-coded English.
