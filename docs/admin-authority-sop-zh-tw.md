...
**重要：精靈與所有 P3B–P3F 授權操作都不能以自己的人員帳號為目標（`self_target_denied`，這是不可變的設計）。第一位 Primary 必須由「另一位」身分建立（見第 17.12 節），要讓原本的緊急操作人（例如創辦人帳號）成為 Primary，必須由該新建立、已獨立完成 TOTP 與 Step-Up 的 Primary，在「創辦人的人員詳情頁」對創辦人執行精靈。**
...
**交叉主體（cross-principal）原則**：Break-glass 啟用者不能替自己授予 Primary。第 17.12 節的第 5–6 步建立的是「另一位」人員（`N`）；`N` 依第 1–3 節獨立登入並完成自己的 TOTP／Step-Up 後，才能對原本的緊急操作人執行第 11 節的精靈（一般跨帳號 P3F／P3E 操作）。完成後依第 17.7 節關閉啟用。正式 Development 已依此完成：創辦人與 `N` 兩位獨立 Primary，且沒有任何進行中的 Break-glass 啟用。
