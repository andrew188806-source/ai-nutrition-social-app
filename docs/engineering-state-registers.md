...
| ADMIN-B | **B1 DONE (14 read-only contracts live on Development); B2 DONE, ACCEPTED (`ADMIN_B2_ACCEPTANCE_PASS`; inventory §1b) — nine Restaurant/Branch pages LIVE; B3 DONE, ACCEPTED (`ADMIN_B3_ACCEPTANCE_PASS`; inventory §1c) — six Menu/Menu Item pages LIVE (item-nutrition reuses the item-detail contract; ingredients DEFERRED). All 15 ADMIN-B routes LIVE, awaiting Planner closure review.** Restaurant operational read model + 15 read-only routes: `restaurants`, `restaurant-detail/-about/-contact`, `-branches`, `-branch-detail/-hours/-contact/-geo/-menu-items`, `-menus`, `-menu-detail`, `-menu-items`, `-menu-item-detail`, `-item-nutrition` (drafts/archived, nutrition status, lifecycle, owner-membership counts) | NEW non-authority read RPCs (sealed-role pattern) | LARGE | ADMIN-AE1 |
| ADMIN-C | **Six canonical routes, ALL LIVE: C1 (`ADMIN_C_ACCEPTANCE_PASS`, inventory §1d) — `menu-management`, `-pending`, `-data-quality`, `nutrition-certification-pending` over four additive read-only contracts (migration `20260920030000`, sealed B1 reader, no new privilege; `data-quality` legitimately empty on Development); C2 (`ADMIN_C2_ACCEPTANCE_PASS`) — `restaurant-item-allergens` and `restaurant-item-certification` as read-only factual pages REUSING the ADMIN-B1 item-detail contract (no migration, no new permission).** Awaiting Planner closure review; C1/C2 commits local, unpushed. | Derived from B | MEDIUM | ADMIN-B |
