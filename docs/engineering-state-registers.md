...
| Admin non-authority operational functionality | **ADMIN-A/B/C/D accepted on Development (`ADMIN_D_ACCEPTANCE_PASS`, ADMIN-D live acceptance 31/31); ADMIN-E not started. Production is untouched.** | Inventory in §7 and `docs/admin-operational-surface-inventory.md`. |
...
| TD-22 | **`ADMIN_FINAL_HARDENING_REQUIRED`: AAL2 step-up receipt clock-skew tolerance / server-vs-DB timestamp authority.** Development acceptance observed the application clock about 1.7 seconds ahead of the database while `staff_step_up_receipts_fixed_window_check` requires `issued_at >= verified_at`; this can intermittently surface as `step_up_broker_unavailable`. A scratchpad harness offset allowed acceptance, but no product/runtime fix exists. | ADMIN-D Development acceptance; deferred without changing the receipt constraint, system clock or product runtime. | `FINAL_HARDENING_REQUIRED` |
| TD-23 | **Old ADMIN-A acceptance fixture residue.** Five active `admin-a.acceptance.*` staff accounts remain with zero entitlements. They carry no effective authority but must be removed during ADMIN-E/final fixture-residue cleanup. | ADMIN-D Development acceptance inventory; no cleanup performed in the ADMIN-D closure patch. | `DEFERRED_CLEANUP` |
