...
| Admin non-authority operational functionality | **ADMIN-A/B/C/D accepted on Development (`ADMIN_D_ACCEPTANCE_PASS`, live 31/31); ADMIN-E1 legacy-route and clock hardening deployed in `31b55d3`; ADMIN-E final live closure LIVE PASS — two independent Primary accounts, no Break-glass window open, the three fixed Demos serve `31b55d3`. Production is untouched.** | Inventory in §7 and `docs/deployment-local-access-matrix.md`. |
...
ADMIN-E final live closure is **LIVE PASS** on Development. The fixed Consumer, Restaurant and Admin URLs are respectively `https://haocu-demo.vercel.app`, `https://ai-nutrition-social-app-restaurant.vercel.app` and `https://tastkind-admin-demo.vercel.app`; all serve `31b55d3101c08a48f868b7988ce8ad6332e2f3fd` from `main`. The Admin Demo runs in `sin1` and needs `TASTKIND_P3H_BROKER_DATABASE_URL` and `TASTKIND_ADMIN_AUDIT_DATA_SOURCE` (`supabase`) besides the Supabase URL, publishable key and staff authority mode (names only). `tastkind_admin_step_up_broker` is the permanent least-privilege step-up broker role. Consumer `meal-photo-analysis` (`verify_jwt = true`) CORS was re-verified 11/11 and the Consumer bundle references only Development; visual acceptance on slow links is still limited by TD-24. Restaurant is linked to Development and a temporary-membership read of real data was accepted. The five old ADMIN-A fixtures were revoked (TD-23). No Production backend access occurred.

**First Primary bootstrap is cross-principal**: `self_target_denied` (P3B–P3F) is unchanged and Break-glass never lets an actor grant itself Primary authority. A finite Break-glass activation is used to link and equip a different legitimate staff identity (`N`), which becomes an independent Primary and then grants the original actor through normal cross-account operations before the activation is closed. Development now has two usable Primary accounts, no live Break-glass activation, and the founder holds sixteen CURRENT operational grants (no PLANNED or deferred keys). Remaining small debts: the Primary Wizard has no self-account warning, and the base-Admin staff roster shows an empty list rather than an explicit no-access note. The exact evidence is in `docs/deployment-local-access-matrix.md`.
