...
| Admin non-authority operational functionality | **ADMIN-A/B/C/D accepted on Development (`ADMIN_D_ACCEPTANCE_PASS`, ADMIN-D live acceptance 31/31); ADMIN-E1 local cleanup/hardening prepared; ADMIN-E2 PARTIAL — `WAITING_FOR_DEPLOYMENT_ENABLEMENT_AND_PRIMARY_RECOVERY`. The three fixed Demos still serve `d88d100` and require the E1 push. Production is untouched.** | Inventory in §7 and `docs/deployment-local-access-matrix.md`. |
...
| --- | --- | --- | --- |
| TD-01 | **Development migration-history drift.** `DEVELOPMENT_MIGRATION_HISTORY_DRIFT_OPEN`, state `NOT_MODIFIED`. The repository holds 139 migrations including ADMIN-D. Development (`tastkind-development`, ref `msbgnnoorsoefuiwluye`) `supabase_migrations.schema_migrations` held 66 rows when last read. Restaurant R2B–R2E, the Admin staff-authority stack, H3/H4 and accepted Admin rounds through ADMIN-D were applied by sending each self-contained migration file to the Management API SQL channel, not by `supabase db push`. The history table therefore does not describe the accepted schema. | Last observed remote history count 66 vs. 139 local files after ADMIN-D. `supabase db push` would attempt migrations whose objects already exist. Schema truth is obtainable by catalog introspection. | `TECH_DEBT` |
...
| TD-03 | **Production has never been enabled.** None of R2B–R2E, the Admin staff-authority stack, or later migrations have been applied to any Production project. Production needs its own `TASTKIND_P3H_BROKER_DATABASE_URL`, `TASTKIND_BREAK_GLASS_PRODUCTION_DATABASE_URL`, and `TASTKIND_ADMIN_AUTHORITY_MODE` for admin-web (required mode: staff; default is a legacy Platform-Admin-table mode). Values are provisioned by the credential owner; only variable *names* appear in the repository. | `docs/engineering-handoff.md` Admin section; `apps/admin-web/config/admin-auth.ts`. | `TECH_DEBT` |
...
| TD-22 | **`LOCAL_HARDENING_DONE_LIVE_PENDING`: AAL2 step-up receipt server-vs-DB timestamp authority.** Development acceptance observed the application clock about 1.7 seconds ahead of the database while `staff_step_up_receipts_fixed_window_check` requires `issued_at >= verified_at`; this can intermittently surface as `step_up_broker_unavailable`. A scratchpad harness offset allowed the original acceptance. ADMIN-E1 now reads database time immediately after successful TOTP verification, before issuance, so ordinary app-ahead drift cannot violate the unchanged `issued_at >= verified_at` constraint. Live acceptance remains E2. | ADMIN-E1 local runtime fix; no migration or SQL constraint change. | `LOCAL_HARDENING_DONE_LIVE_PENDING` |
| TD-23 | **`CLOSED` — old ADMIN-A acceptance fixture residue.** Five obsolete `admin-a.acceptance.*` accounts were verified and canonically revoked on Development during E2. Active staff changed 7 → 2; management audit count 470 → 475; founder authority hashes were unchanged during cleanup. | Accepted E2 live evidence; E2R performs no Development operation. | `CLOSED` |
| TD-24 | **`GLOBAL_QA_PERFORMANCE_DEBT` — Consumer initial font load.** Four approximately 7.1 MB Noto Sans TC web fonts (about 28 MB total) blocked visual rendering in the slow test environment. Consumer in-origin E2E passed, but full visual acceptance did not. | Accepted E2 observation; no font optimization in E2R. | `GLOBAL_QA_PERFORMANCE_DEBT` |
...
| ADMIN-B | **CLOSED / ACCEPTED / PUSHED.** B1 supplies 14 read-only contracts; B2 delivers nine Restaurant/Branch pages; B3 delivers six Menu/Menu Item pages (item-nutrition reuses item detail; ingredients DEFERRED). All 15 ADMIN-B routes are LIVE. | NEW non-authority read RPCs (sealed-role pattern) | LARGE | ADMIN-AE1 |
| ADMIN-C | **CLOSED / ACCEPTED / PUSHED.** Six canonical routes are LIVE: four purpose-built read-only review queues plus `restaurant-item-allergens` and `restaurant-item-certification`, which reuse the B1 item-detail contract. | Derived from B | MEDIUM | ADMIN-B |
| ADMIN-D | **DONE / DEVELOPMENT ACCEPTED — `ADMIN_D_ACCEPTANCE_PASS`, live acceptance 31/31.** `/admin` stays base-Admin while `staff_admin_dashboard_counts_v1()` requires exact `admin.dashboard.counts.read`; `/admin/social/policies` uses bounded `staff_admin_social_policies_v1` over canonical `social_interest_catalog[_label]` and exact `admin.social.policies.read`. Dashboard SQL equality, Social isolation/equality/bounds, negative/write checks, regressions and fixture cleanup passed. One additive migration (`20260921010000`), sealed readers, no client table grant, no write capability; legacy `/` and `/tags` retained. Production is untouched. | Small NEW read RPCs | SMALL | ADMIN-B |
...
- Repository: npm workspaces monorepo — `apps/mobile` (Expo/React Native + Expo Web), `apps/restaurant-web` (Next.js 14), `apps/admin-web` (Next.js 14), `packages/shared`, `packages/services`, `supabase/` (139 migrations including Development-accepted ADMIN-D, 15 Edge Functions), `scripts/`.
...
## ADMIN-E1 local closure and E2 boundary

ADMIN-E1 retires all 22 misleading legacy root experiences: seven redirect to live successors, three are truthful split-route gateways, eight mock-only successors are unavailable, and four consent/deferred roots show no data. The canonical registry and permissions remain unchanged. See `docs/admin-operational-surface-inventory.md` §4a for the route-by-route matrix. The frozen legacy `AdminShell` is inactive. AAL2 now uses a database clock reading after successful TOTP verification; no receipt SQL window, replay, subject, permission or audit contract changed. `.gitattributes` sets LF for text without historical renormalization. Local guards and build evidence belong to the E1 commit report; Development, Demo deployments, and Production were not modified.

ADMIN-E2 is **PARTIAL** (`WAITING_FOR_DEPLOYMENT_ENABLEMENT_AND_PRIMARY_RECOVERY`). The fixed Consumer, Restaurant and Admin URLs are respectively `https://haocu-demo.vercel.app`, `https://ai-nutrition-social-app-restaurant.vercel.app` and `https://tastkind-admin-demo.vercel.app`. All three still auto-deploy from `main` at `d88d1000a26810dfca0f70586f11e594b3a2e44a`; E1 is local/unpushed, so `PUSH_REQUIRED_FOR_FIXED_DEMO`. Consumer Development CORS and `meal-photo-analysis` v45 (`verify_jwt = true`) passed live 11/11, and in-origin E2E passed without a direct browser `api.openai.com` call or observed secret leakage. Visual acceptance remains pending because of TD-24. Restaurant returned `307 → /login?reason=session`, with no exposed client Supabase ref or server secret; Development backend linkage remains **NOT YET LIVE-PROVEN**. Admin `/admin → /admin/login?error=configuration` due to missing/invalid Vercel Supabase runtime configuration; client bundles showed no secret leakage. The five old ADMIN-A fixtures were revoked (TD-23). No Production backend access occurred.

The existing founder account is **NOT PRIMARY READY**: it has active Platform Admin compatibility membership, only `admin_context.read`, `admin_audit.read`, `admin_restaurant_branch.status.write`, and no MFA factor. Final E2 must recover normal `PRIMARY READY` through the eight-step canonical SOP and verify appropriate current operational permissions from the live catalogue; never grant PLANNED/deferred keys. Only if there is zero usable Primary may the documented Break-glass procedure recover the first normal Primary, after which Break-glass must be fully closed. E2R changes documentation and guards only; no authority grant, remote configuration, push or deployment occurs. The exact transition and remaining post-push gates are in `docs/deployment-local-access-matrix.md`.
