...
| ADMIN-C | **Four queue routes DONE, ACCEPTED (`ADMIN_C_ACCEPTANCE_PASS`; inventory §1d): `menu-management`, `-pending`, `-data-quality`, `nutrition-certification-pending` LIVE over four additive read-only contracts (migration `20260920030000`, sealed B1 reader, no new privilege); `data-quality` is legitimately empty on Development.** `restaurant-item-allergens` and `restaurant-item-certification` (status facts only) remain NOT_ENABLED and outside this slice — original 6 routes | Derived from B | MEDIUM | ADMIN-B |
