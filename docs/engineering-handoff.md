...
| Admin non-authority operational functionality | **ADMIN-A/B/C/D are Development-accepted. `ADMIN_D_ACCEPTANCE_PASS`: post-apply verification and live acceptance 31/31 passed; Production is untouched. ADMIN-E has not started.** | Registers §7.6–7.8; `docs/admin-operational-surface-inventory.md` §1e |
...
**Known deferred items**: `admin.management.roles.read` / Bundle-role management; Passkey/WebAuthn (TOTP is the MVP authenticator by design, see P3H markers `TOTP_AAL2_MVP_AUTHENTICATOR_SELECTED` / `PASSKEY_NATIVE_AVAILABLE_BUT_DEFERRED`); destructive MFA-factor recovery (`MFA_FACTOR_DESTRUCTIVE_RECOVERY_DEFERRED`); a "delete another operator's TOTP factor" button in the UI (still requires direct API/database access, documented as such in the SOP rather than left silent). `ADMIN_FINAL_HARDENING_REQUIRED`: add AAL2 step-up receipt tolerance or a single server/DB timestamp authority—the application clock was about 1.7 seconds ahead of the database during ADMIN-D acceptance and could intermittently trigger `step_up_broker_unavailable`; the acceptance-only scratchpad offset is not product code. Five active old `admin-a.acceptance.*` staff accounts with zero entitlements remain for ADMIN-E/final fixture-residue cleanup. The add-new-staff button is done (see Live routes above).
