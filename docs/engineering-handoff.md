...
| Admin non-authority operational functionality | **Next major implementation phase — scope closed by A0 (ADMIN-A → AE1 → B → C → D → E; ADMIN-A, ADMIN-AE1, ADMIN-B1 (Restaurant read foundation), ADMIN-B2 (nine Restaurant/Branch pages LIVE) and ADMIN-B3 (six Menu/Menu Item pages LIVE) done and live-accepted; ADMIN-B awaits Planner closure review, then ADMIN-C)** | Registers §7.6–7.8; `docs/admin-operational-surface-inventory.md` |
