...
**DOC STATUS: CURRENT (living technical description) — ADMIN-E1 remains local/unpushed on baseline `d88d1000a26810dfca0f70586f11e594b3a2e44a`; ADMIN-E2 is PARTIAL, waiting for deployment enablement and Primary recovery.**
...
| Admin non-authority operational functionality | **ADMIN-A/B/C/D are Development-accepted. `ADMIN_D_ACCEPTANCE_PASS`: live acceptance 31/31. ADMIN-E1 remains local; ADMIN-E2 PARTIAL (`WAITING_FOR_DEPLOYMENT_ENABLEMENT_AND_PRIMARY_RECOVERY`). Production is untouched.** | Registers §7.6–7.8; `docs/deployment-local-access-matrix.md` |
...
**Known deferred items**: `admin.management.roles.read` / Bundle-role management; Passkey/WebAuthn (TOTP is the MVP authenticator by design, see P3H markers `TOTP_AAL2_MVP_AUTHENTICATOR_SELECTED` / `PASSKEY_NATIVE_AVAILABLE_BUT_DEFERRED`); destructive MFA-factor recovery (`MFA_FACTOR_DESTRUCTIVE_RECOVERY_DEFERRED`); a "delete another operator's TOTP factor" button in the UI (still requires direct API/database access, documented as such in the SOP rather than left silent). ADMIN-E1 local AAL2 hardening uses the database timestamp immediately after successful TOTP verification; the existing receipt window and SQL gates remain unchanged. The prior app-ahead clock drift caused intermittent `step_up_broker_unavailable` during ADMIN-D acceptance. Live E2 verification remains pending. Five active old `admin-a.acceptance.*` staff accounts with zero entitlements remain for ADMIN-E/final fixture-residue cleanup. The add-new-staff button is done (see Live routes above).

**Development migration history**: `tastkind-development` (`msbgnnoorsoefuiwluye`) is on direct-apply migration history — accepted migrations in this stack were applied via the Management API SQL channel, not `supabase db push`. `DEVELOPMENT_MIGRATION_HISTORY_DRIFT_OPEN` (state `NOT_MODIFIED`): the remote `supabase_migrations.schema_migrations` table (66 rows when last read on 2026-09-19) does not describe the schema, which contains objects from migrations that have no history row; the repository holds 139 migrations including ADMIN-D. **ADMIN-D is applied and accepted: `ADMIN_D_ACCEPTANCE_PASS`, live acceptance 31/31. Production is untouched.** Facts and risk are recorded in `docs/engineering-state-registers.md` TD-01.

**Production**: none of this has ever been applied to Production. Before it can be, Production needs its own `TASTKIND_P3H_BROKER_DATABASE_URL` and `TASTKIND_BREAK_GLASS_PRODUCTION_DATABASE_URL` provisioned (values only, by whoever owns Production credentials — never committed), and the admin-web deployment needs `TASTKIND_ADMIN_AUTHORITY_MODE` configured for staff mode (it defaults to a legacy Platform-Admin-table mode otherwise, which predates and is unrelated to this whole stack).
...
## ADMIN-E1 local handoff

The 22 legacy root routes now render no mock operational records. Seven redirect to live `/admin` pages; three split gateways link to live successors and label deferred portions; twelve routes are explicitly unavailable/deferred. The canonical registry and Admin authority remain unchanged. AAL2 verification now uses the database clock; `.gitattributes` adds LF text policy with no historical normalization. E1 remains local/unpushed. The exact dispositions and E2 transition are in [admin-operational-surface-inventory.md](admin-operational-surface-inventory.md) §4a and [deployment-local-access-matrix.md](deployment-local-access-matrix.md).

ADMIN-E2 is **PARTIAL** (`WAITING_FOR_DEPLOYMENT_ENABLEMENT_AND_PRIMARY_RECOVERY`). The fixed Consumer, Restaurant and Admin Demos are `https://haocu-demo.vercel.app`, `https://ai-nutrition-social-app-restaurant.vercel.app`, and `https://tastkind-admin-demo.vercel.app`; all still serve `d88d1000a26810dfca0f70586f11e594b3a2e44a` from `main`. `PUSH_REQUIRED_FOR_FIXED_DEMO`. Consumer Development `meal-photo-analysis` advanced v44 → v45 with `verify_jwt = true`; CORS live acceptance 11/11 and in-origin E2E passed, without browser calls to `api.openai.com` or observed secret leakage. Full visual acceptance remains pending because four roughly 7.1 MB fonts blocked rendering on the slow test connection (`GLOBAL_QA_PERFORMANCE_DEBT`). Restaurant returned `307 → /login?reason=session`, with no exposed client ref or server secret; Development backend linkage is not yet live-proven. Admin `/admin → /admin/login?error=configuration` because required Vercel runtime configuration is missing/invalid; no client-bundle secret leakage was observed. Five obsolete `admin-a.acceptance.*` accounts were verified and canonically revoked on Development: active staff 7 → 2, management audit 470 → 475, founder authority hashes unchanged.

The existing founder account is **NOT PRIMARY READY**. It has active Platform Admin compatibility membership, only `admin_context.read`, `admin_audit.read`, and `admin_restaurant_branch.status.write`, and no MFA factor. Final E2 targets that same account for the SOP's eight-step `PRIMARY READY` wizard, then verifies appropriate active/current/non-deferred operational permissions through canonical privileged flows. If no usable Primary exists, canonical Break-glass may recover the first normal Primary and must then be fully closed; it cannot become permanent authority. Lower-privilege management reads may show no data without leaking staff rows. E2R changes docs and guards only; no grant, Development/Production/Vercel operation, push, or E2 login acceptance occurs here.
