...
**DOC STATUS: CURRENT (living technical description) — ADMIN-E final live closure: LIVE PASS on Development; the three fixed Demos serve `31b55d3101c08a48f868b7988ce8ad6332e2f3fd`.**
...
| Admin non-authority operational functionality | **ADMIN-A/B/C/D are Development-accepted (`ADMIN_D_ACCEPTANCE_PASS`, live 31/31). ADMIN-E final live closure is LIVE PASS: the fixed Demos serve `31b55d3`, two independent Primary Permission Managers exist, no Break-glass window is open, and Production is untouched.** | Registers §7.6–7.8; `docs/deployment-local-access-matrix.md` |
...
ADMIN-E is closed live on Development (final live closure). The fixed Consumer, Restaurant and Admin Demos are `https://haocu-demo.vercel.app`, `https://ai-nutrition-social-app-restaurant.vercel.app` and `https://tastkind-admin-demo.vercel.app`; all serve `31b55d3101c08a48f868b7988ce8ad6332e2f3fd` from `main`, and the Admin Demo runs in `sin1`. The Admin Vercel Production environment needs `TASTKIND_SUPABASE_URL`, `TASTKIND_SUPABASE_PUBLISHABLE_KEY`, `TASTKIND_ADMIN_AUTHORITY_MODE` (staff), `TASTKIND_P3H_BROKER_DATABASE_URL` and `TASTKIND_ADMIN_AUDIT_DATA_SOURCE` (`supabase`); values are never committed. `tastkind_admin_step_up_broker` is the **permanent** least-privilege P3H runtime login role (issuer membership only, shared transaction pooler); it must not be deleted.

**First Primary bootstrap is cross-principal.** Break-glass cannot let an actor grant itself Primary authority: P3B–P3F `self_target_denied` is an unchanged invariant, and running the Primary Wizard on one's own account fails at step 1. The recovery actor uses a finite Break-glass activation to link and equip a different legitimate staff identity; that independent Primary, with its own TOTP and Step-Up, then grants the original actor through normal cross-account P3F/P3E operations, and the activation is closed. Development now has two usable Primary accounts (the founder and `N`), no live Break-glass activation and sixteen CURRENT founder operational grants; no PLANNED or deferred key was granted. A lower-privilege base Admin is denied every management, Restaurant, nutrition and Social Policies surface and cannot grant; the 22 legacy roots are redirects, gateways or unavailable pages with no mock records. Restaurant and Consumer are linked to Development (Restaurant real-data read accepted with a temporary membership, Consumer CORS 11/11 with `verify_jwt = true`); the Consumer font payload remains `GLOBAL_QA_PERFORMANCE_DEBT`. Known small debts: the Primary Wizard does not warn when opened on the actor's own account, and a base Admin sees an empty staff roster instead of an explicit no-access note.
