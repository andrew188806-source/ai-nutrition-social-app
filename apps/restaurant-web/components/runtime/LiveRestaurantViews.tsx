...
export function LiveLocations({ data }: { data: AwaitedReturn<typeof loadLiveLocations> }) {
  return <Section title={data.restaurant.name} subtitle="僅顯示目前身分經資料庫授權的分店。"><div className="grid gap-4 md:grid-cols-2">{data.branches.map(branch=><Card key={branch.id}><div className="flex justify-between gap-3"><div><h3 className="font-black">{branch.name}</h3><p className="mt-1 text-sm text-stone-500">{branch.address ?? "地址未提供"}</p></div><StatusPill tone={branch.status==="active"?"good":"muted"}>{branch.status}</StatusPill></div><RestaurantOwnerBranchDisplayNameControl branchId={branch.id}/><RestaurantOwnerBranchTemporalControl branchId={branch.id}/></Card>)}</div></Section>;
}
