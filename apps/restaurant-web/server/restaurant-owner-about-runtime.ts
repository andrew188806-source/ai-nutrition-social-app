...
import { createRestaurantOwnerAboutRepository } from "../repositories/supabase/restaurant-owner-about-repository";
import { loadRestaurantAccessContext } from "../runtime/restaurant-access-context";
import {
  parseRestaurantAboutInput, RESTAURANT_OWNER_ABOUT_BODY_LIMIT,
  type RestaurantAboutInput, type RestaurantAboutMutation, type RestaurantAboutPreview
} from "../runtime/restaurant-owner-about";

type Result = RestaurantAboutPreview | RestaurantAboutMutation;
...
export async function previewRestaurantAbout(request: Request) {
  ...
  try {
    ...
    const result = await createRestaurantOwnerAboutRepository().preview(access.restaurant.id);
...
export async function mutateRestaurantAbout(request: Request) {
  if ([...new URL(request.url).searchParams.keys()].length) return json({ state: "invalid_request" });
  const contentLength = request.headers.get("content-length");
  const contentType = request.headers.get("content-type")?.split(";", 1)[0]?.trim().toLowerCase();
  if (contentType !== "application/json" || (contentLength !== null
    && (!/^[0-9]+$/.test(contentLength)
      || Number(contentLength) > RESTAURANT_OWNER_ABOUT_BODY_LIMIT))) {
    return json({ state: "invalid_request" });
  }
  const auth = await authenticate();
  if (typeof auth === "string") return json({ state: auth });
  let input: RestaurantAboutInput | null;
  try {
    const body = await request.text();
    if (new TextEncoder().encode(body).byteLength > RESTAURANT_OWNER_ABOUT_BODY_LIMIT) {
      return json({ state: "invalid_request" });
    }
    input = parseRestaurantAboutInput(JSON.parse(body));
  ...
  try {
    ...
    const result = await createRestaurantOwnerAboutRepository().mutate(access.restaurant.id, input);
