...
async function auth(){if(getRestaurantDataSourceConfig().dataSource!=="supabase")return"dependency_unavailable" as const;try{return await getVerifiedRestaurantClaims()??"unauthenticated" as const}catch{return"dependency_unavailable" as const}}
export async function previewPublicSocial(request:Request,providerValue:unknown){
  ...
  const a=await auth();if(typeof a==="string")return json({state:a});
...
export async function mutatePublicSocial(request:Request,providerValue:unknown){
  ...
  const a=await auth();if(typeof a==="string")return json({state:a});let input:PublicSocialInput|null;
  ...
  try{const access=await loadRestaurantAccessContext();if(access.state!=="selected")return json({state:access.state==="missing-identity"?"unauthenticated":"permission_denied"});
    const result=await createRestaurantOwnerPublicSocialLinksRepository().mutate(a.subject,access.restaurant.id,providerValue,input);
