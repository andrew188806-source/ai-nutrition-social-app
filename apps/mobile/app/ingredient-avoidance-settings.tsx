...
import {
  isConsumerIngredientAvoidanceSettingsReady,
  useConsumerIngredientAvoidanceSettings
} from "../features/consumer-ingredient-avoidance-settings";
...
export default function IngredientAvoidanceSettingsScreen() {
  const router = useRouter();
  const runtime = useConsumerRuntime();
  const copy = zhTW.mobile.ingredientAvoidanceSettings;
  const controller = useConsumerIngredientAvoidanceSettings(
    runtime.state.actorKey,
    ...
    return (
      <View style={styles.centered}>
        <Card style={styles.noticeCard}>
          ...
          {!signedOut
            ? <PrimaryButton label={copy.retry} onPress={() => void controller.retryLoad()} />
            : null}
  ...
  if (!isConsumerIngredientAvoidanceSettingsReady(state)) return null;
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Card>
        <SectionHeader title={copy.title}
          subtitle={state.draft.length === 0 ? copy.empty : undefined} />
      ...
      {state.unresolvedSelectionCount > 0 ? (
        <Card tone="blush">
          <SectionHeader title={copy.unresolvedTitle} subtitle={copy.unresolvedBody} />
        </Card>
      ) : null}
      ...
      <PrimaryButton
        ...
        disabled={saving || !state.dirty || state.unresolvedSelectionCount > 0}
...
function BackButton({ disabled, onPress }: { disabled: boolean; onPress(): void }) {
  return <Pressable disabled={disabled} onPress={onPress} style={styles.backButton}>
    <Text style={styles.backText}>{zhTW.mobile.ingredientAvoidanceSettings.back}</Text>
  </Pressable>;
}
...
const styles = StyleSheet.create({
  ...
  option: { flexDirection: "row", alignItems: "center", gap: 12, borderWidth: 1,
    borderColor: colors.line, borderRadius: 14, paddingHorizontal: 14, paddingVertical: 12,
    backgroundColor: colors.card },
  optionSelected: { borderColor: colors.primaryDeep, backgroundColor: colors.primarySoft },
  checkbox: { width: 23, height: 23, borderRadius: 7, borderWidth: 1,
  ...
  success: { color: colors.primaryDeep, textAlign: "center", fontFamily: fonts.bold,
    fontWeight: "800" },
