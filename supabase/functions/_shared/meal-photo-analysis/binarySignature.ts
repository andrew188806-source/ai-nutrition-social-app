...
// packages/shared/src/domain/meal-photo-analysis/binarySignature.ts by
...
import type { SupportedMealPhotoExtension, SupportedMealPhotoMimeType } from "./types.ts";

// MI-E-C4: promoted verbatim from apps/mobile/features/meal-photo-upload/binarySignature.ts
// (MI-E-C3-R1) so Mobile's upload-time validation and the Edge Function's server-side
// revalidation are provably the same rule, not two independently maintained copies that could
// drift. Mobile's own module now only re-exports this. Pure — no Expo/React Native dependency,
// so it is safe for the Deno Edge Function runtime to import directly.
