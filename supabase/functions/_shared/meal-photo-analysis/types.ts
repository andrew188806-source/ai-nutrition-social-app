...
// ---- SupportedMealPhotoMimeType / SupportedMealPhotoExtension: canonical source ----
//
// MI-E-C4: promoted here from apps/mobile/features/meal-photo-upload/types.ts (single canonical
// definition, re-exported not redefined by Mobile) because the Edge Function's server-side binary
// revalidation (binarySignature.ts, same folder) must use the exact same accepted-format
// vocabulary Mobile's upload path already validates against — two independently maintained copies
// would drift the moment either side added a format the other didn't know about.
...
// (supabase/migrations/20260725030000_meal_photo_analysis_private_storage_bucket.sql). Promoted
// here (MI-E-C4) from apps/mobile/features/meal-photo-upload/types.ts, which now re-exports this
// same constant rather than defining a second copy, so the Edge Function's server-side size
// revalidation always agrees with Mobile's own client-side preflight check.
...
// MI-E-C4: shared bounds the runtime validators (requestValidation.ts, providerOutputSchema.ts,
// responseValidation.ts) enforce — kept as named constants rather than magic numbers scattered
// across those files, and reused identically by the guard's own checks so the guard can assert
// against the same numbers the validators actually use, not a second hand-copied set.
export const MEAL_PHOTO_ANALYSIS_MIN_CANDIDATES = 1;
export const MEAL_PHOTO_ANALYSIS_MAX_CANDIDATES = 3;
export const MEAL_PHOTO_ANALYSIS_MAX_COMPONENTS_PER_CANDIDATE = 12;
export const MEAL_PHOTO_ANALYSIS_MAX_OBSERVED_NAME_LENGTH = 200;
export const MEAL_PHOTO_ANALYSIS_MAX_UNCERTAINTY_REASON_CODES = 8;

// Fixed, closed vocabulary for uncertainty reasons — the model is instructed (prompt v1, server
// side) to choose only from this set via a structured-output enum constraint, and this list is
// re-checked independently by local runtime validation rather than trusting that constraint alone.
export const MEAL_PHOTO_ANALYSIS_UNCERTAINTY_REASON_CODES = [
  "photo_unclear",
  "poor_lighting",
  "partial_view",
  "multiple_items_overlapping",
  "unfamiliar_dish",
  "packaging_obscures_contents",
  "possible_multiple_servings",
  "low_confidence_portion_estimate"
] as const;
export type MealPhotoAnalysisUncertaintyReasonCode = (typeof MEAL_PHOTO_ANALYSIS_UNCERTAINTY_REASON_CODES)[number];

// Stable, machine-readable error-code vocabulary shared by the Edge Function's HTTP error
// envelope and MealPhotoAnalysisResponseV1.safeUserFacingErrorCode — one vocabulary, not two, so a
// client only ever needs to branch on one set of strings regardless of which surface reported it.
export const MEAL_PHOTO_ANALYSIS_ERROR_CODES = [
  "authentication_required",
  "invalid_request",
  "invalid_image_object_ref",
...
export type MealPhotoAnalysisErrorCode = (typeof MEAL_PHOTO_ANALYSIS_ERROR_CODES)[number];
