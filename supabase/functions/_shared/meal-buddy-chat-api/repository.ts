...
import type { InternalChatConversation, InternalChatConversationRow, InternalChatMessage, InternalChatMessageRow, InternalChatProfileRow, MealBuddyChatCounterpart } from "./types.ts";
...
export interface MealBuddyChatRepository {
  open(actorUserId: string, relationshipId: string): Promise<InternalChatConversation>;
...
export class ExecutorMealBuddyChatRepository implements MealBuddyChatRepository {
  ...
  async open(actor: string, relationship: string) { return this.transport.withTransaction(async (tx) => this.conversation(actor, await tx.query(OPEN, [actor, relationship]), tx)); }
