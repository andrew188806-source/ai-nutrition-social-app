...
-- MI-E-C5-B1-R3: additive persistence correction for accepted v3 finalizations.
--
-- meal_corrections.verification_status is the existing canonical authority for
-- user_confirmed/user_corrected. This migration makes the existing correction ledger explicitly
-- capable of storing a confirmation event without pretending that a name, nutrition value,
-- ingredient, or portion changed. The event has:
--   correction_type = 'confirmation'
--   before_value = NULL
--   after_value = {"confirmationMode":"accepted"}
--   verification_status = 'user_confirmed'
--
-- The NOT VALID constraint preserves every historical row without a table scan while enforcing
-- the formal event shape for every newly inserted/updated user_confirmed row. The partial unique
-- index gives database-level at-most-once evidence per analysis. Existing v1/v2 rows and the
-- corrected/manual v3 paths remain unchanged.

ALTER TABLE public.meal_corrections
  ADD CONSTRAINT meal_corrections_user_confirmed_event_shape_check
  CHECK (
    verification_status <> 'user_confirmed'
    OR correction_type <> 'confirmation'
    OR (
      correction_type = 'confirmation'
      AND before_value IS NULL
      AND after_value = '{"confirmationMode":"accepted"}'::jsonb
      AND correction_reason IS NULL
    )
  )
  NOT VALID;

CREATE UNIQUE INDEX meal_corrections_accepted_confirmation_unique
  ON public.meal_corrections (meal_analysis_id)
  WHERE correction_type = 'confirmation'
    AND verification_status = 'user_confirmed';

COMMENT ON CONSTRAINT meal_corrections_user_confirmed_event_shape_check
  ON public.meal_corrections IS
  'MI-E-C5-B1-R3: user_confirmed confirmation rows are explicit accepted events, never fabricated edited values; other existing user_confirmed correction semantics remain valid.';

COMMENT ON INDEX public.meal_corrections_accepted_confirmation_unique IS
  'MI-E-C5-B1-R3: at most one accepted user-confirmation event may exist for a meal analysis.';

COMMENT ON TABLE public.meal_corrections IS
  'Consumer-owned confirmation-and-correction ledger. correction_type=confirmation with verification_status=user_confirmed is an accepted-confirmation event, not an edited meal value.';

COMMENT ON COLUMN public.meal_corrections.correction_type IS
  'Ledger event discriminator. Existing correction types retain their correction semantics; confirmation denotes an explicit non-editing confirmation event.';

COMMENT ON COLUMN public.meal_corrections.verification_status IS
  'Verification state of this ledger row. user_confirmed + confirmation means the user accepted the linked AI result unchanged; user_corrected remains a real correction and never implies verified nutrition.';
...
COMMENT ON FUNCTION public.finalize_current_user_meal_identification_v1(
  ...
  'Single authenticated atomic meal-identification finalization RPC. Accepts legacy v1, v2 (Catalog/personal-unresolved) and v3 (MI-E-C4/C5-A real-AI candidate accept/correct/manual, linking an EXISTING meal_analyses row); accepted v3 writes one explicit user_confirmed event.';
