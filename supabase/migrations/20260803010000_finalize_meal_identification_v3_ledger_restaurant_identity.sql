...
-- MI-E-C5-R7-B1-R2: project the v3 canonical restaurant context onto the finalization ledger.
--
-- 20260802010000 introduced the optional v3 restaurant context and persisted it on
-- meal_record_items and inside command_snapshot, but left meal_identification_finalizations'
-- own restaurant_id/branch_id columns NULL. A Development deployment-readiness audit found that
-- those ledger columns already exist (created in 20260724020000) and are already populated by the
-- v1/v2 branch, so v3 would have produced rows where the item and the snapshot named a venue while
-- the ledger column beside them said nothing — a version-dependent meaning for the same column.
--
-- This migration writes them for v3 as well, from the SAME validated local variables, so all three
-- durable representations of the venue are populated in one transaction from one source.
--
-- Scope: function body only, and within it only the v3 ledger INSERT (plus the function comment).
-- The signature, return type, language, security context, search_path and grants are unchanged, so
-- existing privileges survive CREATE OR REPLACE and PostgREST needs no schema reload. The v1 and v2
-- branches are carried over byte-identically. Nothing else in v3 moves: the two accepted key sets,
-- the Model A canonical-text rejection, the ordering that puts rejection before the fingerprint and
-- catalog existence checks after the replay branch, the fingerprint contents, the replay/conflict
-- semantics and the meal item projection are all exactly as frozen in 20260802010000.
--
-- No column, constraint, index, RLS policy or Edge Function change: the ledger columns being
-- populated here already exist and are already nullable.
...
COMMENT ON FUNCTION public.finalize_current_user_meal_identification_v1(
  ...
  'Single authenticated atomic meal-identification finalization RPC. Accepts legacy v1, v2 (Catalog/personal-unresolved) and v3 (MI-E-C4/C5-A real-AI candidate accept/correct/manual, linking an EXISTING meal_analyses row); accepted v3 writes one explicit user_confirmed event. v3 additionally accepts an optional canonical restaurant context (restaurantId + branchId as a pair, validated against the restaurant/branch base tables) which is persisted identically on the meal record item, on the finalization ledger row and inside the stored command snapshot.';
