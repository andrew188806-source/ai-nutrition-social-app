...
-- MI-E-C5-R7-B1: canonical restaurant context for the v3 finalization branch.
--
-- Before this migration the v3 branch hard-coded NULL for every catalog identity on the meal
-- record item, so a meal eaten at a restaurant was indistinguishable from one cooked at home.
-- The R7-A-R1 mobile session already carries an ID-ONLY canonical context (restaurantId +
-- branchId, no display name anywhere); this migration is the durable half of that contract.
--
-- Scope and deliberate non-goals:
--   * Function body only. The signature, RETURNS type, volatility, security context and
--     search_path are unchanged, so existing grants survive CREATE OR REPLACE and PostgREST
--     needs no schema reload. The v1 and v2 branches are carried over unmodified.
--   * No new columns. meal_record_items.restaurant_id / branch_id already exist (see
--     20260712130400) and create_current_user_meal_record already reads restaurantId/branchId
--     from the item json (see 20260720010000). meal_identification_finalizations gets NO
--     restaurant columns: its command_snapshot already stores the whole command, and a second
--     durable copy would be a second authority.
--   * No new constraint, index or RLS policy. meal_record_items_restaurant_idx already exists.
--   * No Edge Function change: meal-photo-analysis never touches finalization.
--
-- Compatibility: the v3 top-level key set may be either the original 8 keys or those 8 plus
-- restaurantId AND branchId. Because the ids are OMITTED rather than sent as nulls when there is
-- no venue, a client built before this migration keeps validating against the unchanged 8-key
-- path, and this function accepts both shapes.
--
-- Idempotency is unchanged and needs no new comparison: the server request fingerprint embeds
-- the entire p_finalization, so replaying one client_request_id with a different restaurant
-- context differs from the stored fingerprint and raises IDEMPOTENCY_KEY_CONFLICT before any
-- write, while an identical replay returns the original durable result untouched.
...
COMMENT ON FUNCTION public.finalize_current_user_meal_identification_v1(
  ...
  'Single authenticated atomic meal-identification finalization RPC. Accepts legacy v1, v2 (Catalog/personal-unresolved) and v3 (MI-E-C4/C5-A real-AI candidate accept/correct/manual, linking an EXISTING meal_analyses row); accepted v3 writes one explicit user_confirmed event. v3 additionally accepts an optional canonical restaurant context (restaurantId + branchId as a pair, validated against the restaurant/branch base tables) which is persisted on the meal record item.';
