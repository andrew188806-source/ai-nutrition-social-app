...
-- MI-E-C5-B1: backward-compatible extension of the single authoritative
-- finalize_current_user_meal_identification_v1 RPC to support the MI-E-C4/C5-A
-- real-AI analysis pipeline. Adds a new "meal-identification-finalization-v3"
-- contract branch that links a NEW meal record to an EXISTING meal_analyses row
-- (the one already created by the meal-photo-analysis Edge Function) instead of
-- ever inserting a second, differently-shaped analysis row. The v1 and v2
-- branches below are byte-identical to the currently deployed function body
-- (see 20260724030000_meal_source_record_timing_contract_correction.sql) — only
-- a new v3 IF block is inserted between the existing v1 delegation and the
-- existing "must be v2" guard. No table is replaced, no historical column is
-- dropped, no anon/authenticated direct table grant is widened, no Storage or
-- Edge Function change is included, and no second RPC is created.

-- ============================================================================
-- PART 1: additive ledger shape for v3 (ai_candidate) finalizations
-- ============================================================================

ALTER TABLE public.meal_identification_finalizations
  ADD COLUMN IF NOT EXISTS confirmation_mode text;

-- meal_identification_finalizations_version_check (from
-- 20260724030000_meal_source_record_timing_contract_correction.sql) only permitted v1/v2 — must
-- be additively widened to also permit v3, or every v3 INSERT below would fail a real Postgres
-- CHECK violation (SQLSTATE 23514) rather than one of this function's own typed error codes.
ALTER TABLE public.meal_identification_finalizations
  ...
  CHECK (
    contract_version IN (
      ...
      'meal-identification-finalization-v3'
    )
  );
...
COMMENT ON COLUMN public.meal_identification_finalizations.confirmation_mode IS
  'MI-E-C5-B1: accepted | corrected | manual, server-derived (never trusted from the client). Only set when selection_kind = ai_candidate; NULL for legacy v1/v2 catalog_item/personal_unresolved rows.';

-- ============================================================================
-- PART 2: CREATE OR REPLACE FUNCTION — same name, same signature, same grants
-- (Postgres preserves grants across a same-signature CREATE OR REPLACE). The
-- v1 delegation block and the v2 body below are copied verbatim from the
-- currently deployed function; only the new v3 IF block is inserted between
-- them, and only new v3-local variables are added to the DECLARE list.
-- ============================================================================
